...
type IconType = 'search' | 'edit' | 'delete' | 'home' | 'settings' | 'user' | 'heart' | 'check';
...
const SketchIconography: React.FC<SketchIconographyProps> = ({
  ...
  // Add slight randomness to the path for a hand-drawn effect
  const getWobblyPath = (path: string) => {
    // In a real implementation, we'd add some random jitters
    // But for demonstration, we'll just return the path
    return path;
  };

  return (
    <div className={`sketch-icon ${className}`} style={{ width: size, height: size }}>
      <svg 
      ...
      >
        <path d={getWobblyPath(iconPaths[icon])} />
      </svg>
    </div>
  );
};
